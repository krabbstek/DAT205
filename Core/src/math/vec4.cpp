...
namespace core {
	...
	vec4 CORE_API operator+(vec4 vec, float f)
	{
		return vec.Add(f);
	}

	vec4 CORE_API operator+(float f, vec4 vec)
	{
		return vec.Add(f);
	}

	vec4 CORE_API operator+(vec4 left, const vec4& right)
	{
	...
	CORE_API std::ostream& operator<<(std::ostream& os, const vec4& vec)
	{
