...
namespace core {
	...
	vec3 CORE_API operator+(vec3 vec, float f)
	{
		return vec.Add(f);
	}

	vec3 CORE_API operator+(float f, vec3 vec)
	{
		return vec.Add(f);
	}

	vec3 CORE_API operator+(vec3 left, const vec3& right)
	{
	...
	std::ostream& CORE_API operator<<(std::ostream& os, const vec3& vec)
	{
