...
void main()
{
	viewSpacePosition_FS = interpolate3D(worldSpacePosition_TES[0], worldSpacePosition_TES[1], worldSpacePosition_TES[2]);
	viewSpacePosition_FS = (u_ViewMatrix * vec4(viewSpacePosition_FS, 1.0)).xyz;
	
	viewSpaceNormal_FS = interpolate3D(worldSpaceNormal_TES[0], worldSpaceNormal_TES[1], worldSpaceNormal_TES[2]);
	viewSpaceNormal_FS = normalize(mat3(u_ViewMatrix) * viewSpaceNormal_FS);

	texCoord_FS = interpolate2D(texCoord_TES[0], texCoord_TES[1], texCoord_TES[2]);
